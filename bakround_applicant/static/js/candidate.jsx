...
export default class Candidate extends React.Component {
    ...
    showExpandedContent() {
        if (!this.state.expanded_data) {
            ...
            Networking.json("GET", "/employer/profile_summary/" + this.props.id, {
                employer_job_id: this.props.job_id
    ...
    restrictProfile(event) {
    	...
    	let _this = this;
        Networking.json("POST", "/employer/restrict_profile/" + this.props.id)
    ...
    getCandidateStatusContent() {
        ...
        Networking.json("GET", "/employer/candidate_status/" + this.props.candidate_id)
                  .then(data => _this.setState({candidate_status: data}));
    
        Networking.json("GET", "/employer/fetch_possible_statuses")
                  .then(data => _this.setState({possibleStatuses: data}));

        Networking.json("GET", "/employer/fetch_possible_reject_reasons")
    ...
    submitStatusForm() {
        ...
        let _this = this;
        Networking.json("POST", "/employer/change_candidate_status/" + this.props.candidate_id, {
    ...
    submitFeedback() {
        ...
        params.employer_job_id = this.props.job_id;
        ...
        Networking.json("POST", "/employer/submit_feedback_post", params);
    	...
    	if (this.state.expanded) {
    	    ...
    	    download_profile_button = (
                <div style={{'textAlign':'right', 'margin': '5px 0px'}}>
    	            <a target='_blank'
    	               href={'/profile/pdf/' + this.props.id + '?hide_bscore=t&ejid=' + this.props.job_id}
    	               className='btn-primary send-intro-button'
    	               onClick={this.downloadProfileButtonClicked}
    	               >&nbsp;Download profile</a>
                    <a target='_blank'
                       href={`/employer/preview_intro/${this.props.job_id}?profile_id=${this.props.id}`}
                       className='btn-primary send-intro-button'
                       >&nbsp;Preview Intro</a>
    	        </div>
            );
                    ...
                    <div className='candidate-info-container line'>
                        <div style={{ display: 'flex', flexDirection: 'row' }}>
                            ...
                            <div className='candidateTitle candidateContent'
                                 ...
                                 data-place='right'
                                 data-delay-show='700'
                                 data-tip="Once you contact the candidate and they respond, you'll be able to see their full name and contact info">{this.props.accepted ? this.props.first_name + ' ' + this.props.last_name : this.props.initials}</div>
                        </div>
