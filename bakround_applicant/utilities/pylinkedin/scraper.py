...
class LinkedinItem(object):
    ...
    def __init__(self, url=None, html_string=None, crequest=None):
        ...
        #             self.url))
        # Header path
        self.xp_header = extract_one(self.tree.xpath('//section[contains(@class, "pv-top-card-section")]'))

        # Experiences path
        self.xp_experiences = self.tree.xpath('//section[contains(@class, "experience-section")]/ul/li')
        # Projets
        ...
        # Education
        self.xp_educations = self.tree.xpath('//section[contains(@class, "education-section")]/ul/li')
        # Certifications
        self.xp_certifications = self.tree.xpath('//h4[contains(@class, "pv-accomplishment-entity__title")]')
        # Courses
        ...
        # Skills
        self.xp_skills = self.tree.xpath('//div[contains(@class, "pv-skill-entity__pill-contents")]')
        # Organization
    ...
    def create_raw_path(self):
        ...
        # Header path
        self.xp_header_raw = extract_one(self.tree.xpath('//div[contains(@class, "pv-top-card-section__information")]'))
        # Experiences path
    ...
    def name(self):
        ...
        return extract_one(self.get_xp(self.xp_header, './/h1/text()'))
    ...
    def current_title(self):
        ...
        return extract_one(self.get_xp(self.xp_header, './/h2[contains(@class, "pv-top-card-section__headline")]/text()'))

    @property
    def current_location(self):
        """ Return current location """
        return extract_one(self.get_xp(self.xp_header, './/h3[contains(@class, "pv-top-card-section__location")]/text()'))
    ...
    def skills(self):
        ...
        if len(self.xp_skills) > 0:
            ...
            return [extract_one(self.get_xp(s, './/span[contains(@class, "pv-skill-entity__skill-name")]/text()')) for s in self.xp_skills]
        else:
            return []
    ...
    def summary(self):
        ...
        return self.get_clean_xpath('string(//p[contains(@class, "pv-top-card-section__summary-text")])')
    ...
    def experiences(self):
        ...
        if len(self.xp_experiences) > 0:
            for experience in self.xp_experiences:
                data = {}
                data['job_title'] = extract_one(
                    self.get_xp(experience, './/h3//text()'))
                # data['jobtitle_url'] = extract_one(
                #     self.get_xp(experience, './/h4[@class="item-title"]/a/@href'))
                data['company'] = extract_one(
                    self.get_xp(experience, './/h4/span[@class="pv-entity__secondary-title"]//text()'))
                ...
                data['description'] = ' '.join(self.get_xp(
                    experience, './/p[contains(@class,"pv-entity__description")]/text()'))

                data['location'] = extract_one(self.get_xp(
                    experience, './/h4[contains(@class, "pv-entity__location")]/span[2]/text()'))

                dates = extract_one(self.get_xp(
                    experience, './/h4[contains(@class, "pv-entity__date-range")]/span[2]/text()')).split(' – ')
    ...
    def educations(self):
        ...
        if len(self.xp_educations) > 0:
            for school in self.xp_educations:
                data = {}
                data['university_name'] = extract_one(
                    self.get_xp(school, './/h3//text()'))
                ...
                data['description'] = extract_one(self.get_xp(
                    school, './/p[contains(@class,"pv-entity__description")]/text()'))
                # if data['description'] is not None:
                #     data['degree'] = get_list_i(data['description'].split(','), 0)
                #     data['major'] = get_list_i(data['description'].split(','), 1)
                # else :
                data['degree'] = extract_one(self.get_xp(
                    school, './/p[contains(@class,"pv-entity__degree-name")]/span[2]/text()'))
                data['major'] = extract_one(self.get_xp(
                    school, './/p[contains(@class,"pv-entity__fos")]/span[2]/text()'))

                start_date = extract_one(self.get_xp(
                    school, './/p[contains(@class, "pv-entity__dates")]/span[2]/time[1]/text()'))
                end_date = extract_one(self.get_xp(
                    school, './/p[contains(@class, "pv-entity__dates")]/span[2]/time[2]/text()'))
    ...
    def certifications(self):
        ...
        if len(self.xp_certifications) > 0:
            for certification in self.xp_certifications:
                cert = extract_one(self.get_xp(certification, './text()'))
